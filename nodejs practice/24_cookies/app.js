...
app.use(cookieParser('12345-67890-09876-54321')); // pass securet key
...
function auth(req, res, next) {
  ...
  if (!req.signedCookies.user) {
    ...
    if (username == 'shubham' && password == "password") {
      //now user is authorized hence set cookies
      res.cookie('user','shubham',{signed : true});
      next();
    }
    else {
      var err = new Error('You are not authenticated');
    ...
    if (req.signedCookies.user == 'shubham'){
      next();
    }
    else{
      var err = new Error('You are not authenticated');
